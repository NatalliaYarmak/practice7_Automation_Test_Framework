...
public final class Logger {
	...
	private static PropertiesResourceManager localManager = new PropertiesResourceManager(String.format(
			"localization/loc_%1$s.properties", System.getProperty(AQA_LOCALE, DEF_LOCALE.toString()).toLowerCase()));
	...
	public void logTestName(final String testName) {
		if (logSteps) {
			String formattedName = String.format("=====================  %1$s: '%2$s' =====================",
					getLoc("loc.logger.test.case"), testName);
	...
	public void logTestEnd(final String testName) {
		if (logSteps) {
			info("");
			String formattedEnd = String.format("***** %1$s: '%2$s' %3$s! *****", getLoc("loc.logger.test.case"),
					testName, getLoc("loc.logger.test.passed"));
			String stars = "";
	...
	public void logPrereqName(final String testName) {
		if (logSteps) {
			info(String.format("=====================  %1$s: '%2$s' =====================",
					getLoc("loc.logger.test.prerequisite.case"), testName));
			info("----------------------------------------------------------------------------------------------------");
	...
	public void logPrereq(final String testName) {
		if (logSteps) {
			info("----------------------------------------------------------------------------------------------------");
			info(String.format("=====================  %1$s: '%2$s' =====================",
					getLoc("loc.logger.test.prerequisite.case.creating"), testName));
			info("----------------------------------------------------------------------------------------------------");
		}
	}
	...
	public void logPrereqEnd(final String testName) {
		if (logSteps) {
			info(String.format("===================== %1$s: '%2$s' =====================",
					getLoc("loc.logger.test.prerequisite.case.succes"), testName));
			info("----------------------------------------------------------------------------------------------------");
		}
	}

	// All methods below are using for creation messages with a different level
	// of importance
	/**
	 * Debug log
	 * 
	...
	public void debug(final String message) {
		...
		// msg = "<div class=\"skipped\">" + message + "</div>"; // yellow color
		// from reportng css
		// Reporter.log(msg + "<br>");
	}
