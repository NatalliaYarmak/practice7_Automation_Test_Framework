...
public abstract class BaseElement extends BaseEntity {
	 ...
	 * {@link BaseEntity#stageController}
	 * 
	 * @param locatorStageA
	 *            First locator
	 * @param locatorStageB
	...
	/**
	 * Performing troubleshooting via changing active locator, output log and
	 * report.
	 */
	private void performTroubleShooting() {
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				info("Re-try with locator: \t" + locator.toString() + String
						.format(new String(new char[i]).replace('\0', ' ') + " :%s", result ? "FOUND!" : "NOT FOUND"));
				if (result) {
					break;
				}
			}
	...
	public boolean isPresent(int timeout) {
		...
		try {
			browser.getDriver().manage().timeouts().implicitlyWait(Integer.valueOf(browser.getTimeoutForCondition()),
					TimeUnit.SECONDS);
			return element.isDisplayed();
		} catch (Exception e) {
			warn(e.getMessage());
